...
abstract contract EscrowOperations is ICryplioEscrow, EscrowAuth, ReentrancyGuard, EIP712 {
     ...
     * @dev FINANCIAL: Deducts 0.25% platform fee
     *      ACCESS: Only callable by authorized admin signers
     *      REENTRANCY: Protected by nonReentrant modifier
     *      SECURITY: Used for dispute resolution or after expiry by admin
     * @param tradeId Trade identifier
     * @return bool Always returns true on success
     * @custom:throws EscrowTypes.NotAuthorizedSigner if caller is not an admin
     * @custom:throws EscrowTypes.InvalidStatus if escrow not in Locked state
     */
